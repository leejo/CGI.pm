...
$ENV{HTTP_LOVE}       = 'true';
$ENV{CONTENT_LENGTH}  = length( $xml_string );
...
is $q->param(), 3,"CGI::param()";
is join(' ',sort $q->param()), 'DELETEDATA game weather',"CGI::param()";
...
is $q->query_string, 'DELETEDATA=xml;game=chess;game=checkers;weather=dull;foo=bar',"CGI::query_string() redux";
...
is $q->self_url,
     'http://the.good.ship.lollypop.com:8080/cgi-bin/foo.cgi/somewhere/else?DELETEDATA=xml;game=chess;game=checkers;weather=dull;foo=bar',
     "CGI::url()";
...
is $q->url(-relative=>1,-path=>1,-query=>1),
     'foo.cgi/somewhere/else?DELETEDATA=xml;game=chess;game=checkers;weather=dull;foo=bar',
     'CGI::url(-relative=>1,-path=>1,-query=>1)';
$q->delete('foo');
ok !$q->param('foo'),'CGI::delete()';

is( $q->param( 'DELETEDATA' ),$xml_string,'DELETEDATA' );
