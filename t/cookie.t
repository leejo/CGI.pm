...

{
  ...
  ok( eq_set( [ keys %result ], [qw(foo bar baz qux)] ), "expected cookies extracted" );
...

{
  ...
  like( $c->as_string, '/HttpOnly/', "Stringified cookie contains HttpOnly" );

  like( $c->as_string, '/SameSite=Strict/', "Stringified cookie contains normalized SameSite" );

  like( $c->as_string, '/Priority=High/', "Stringified cookie contains normalized Priority" );

  like( $c->as_string, '/Partitioned/', "Stringified cookie contains Partitioned" );
  ...
  ok( $c->as_string !~ /HttpOnly/, "Stringified cookie does not contain HttpOnly" );

  ok( $c->as_string !~ /SameSite/, "Stringified cookie does not contain SameSite" );

  ok( $c->as_string !~ /Priority/, "Stringified cookie does not contain Priority" );

  ok( $c->as_string !~ /Partitioned/, "Stringified cookie does not contain Partitioned" );
}
...
APACHEREQ: {
  ...
  ok eq_array( $r->{check}, [ 'Set-Cookie', $c->as_string ] ), 'bake() should call err_headers_out->add()';
